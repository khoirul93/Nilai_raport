...
with DM.q_akademis do
  ...
  Close;
  SQL.Clear;
  SQL.Text:='select ROW_NUMBER() over (order by akademis_kd ) as NO, akademis.akademis_kd,akademis.ta,akademis.smt, siswa.siswa_nis, siswa.siswa_nama,kepribadian.kep_kd,kepribadian.kep_jenis,akademis.nilai,akademis.akademis_keterangan from akademis,siswa, kepribadian ' +
...
f_input_akademis.cb_nis.Enabled:=True;
...
begin
  ...
  begin
    ...
    DM.ADOQuery1.SQL.Add('select max(right(akademis_kd,3))as kode from akademis');
    DM.ADOQuery1.Open;
    f_input_akademis.edit_kd.Text:='AKD00'+IntToStr(DM.ADOQuery1['kode']+1);
  end
...
begin
  ...
  f_input_akademis.cb_tahunajrn.Text:=FieldByName('ta').AsString;
  f_input_akademis.cb_semster_mapel.Text:=FieldByName('smt').AsString;
